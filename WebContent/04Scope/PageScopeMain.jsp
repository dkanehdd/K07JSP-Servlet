...
모든타입의 객체를 저장할수 있다. Object타입으로 자동 형변환되어
저장되고, 사용시에는 각 객체로 형변환후 사용하면 된다.
차후 EL과 JSTL에서도 주로 해당 영역을 사용하게된다.

-page영역
	페이지 영역에 저장된 속성은 해당 페이지에서만 공유 되고,
	페이지를 벗어나는 순간 소멸된다.
*/
//페이지 영역에 데이터를 저장
...
<body>
	...
	*/
	...
	String m1Str = String.format("아이디:%s, 비번:%s, 이름:%s, 가입일:%s",
				m1.getId(), m1.getPass(), m1.getName(), m1.getRegidate());
	...
	<ul>
		...
		<li>MemberDTO타입2 : 아이디:<%=m2.getId() %>, 
			비번:<%=m2.getPass() %>, 이름:<%=m2.getName() %>
			, 가입일:<%=m2.getRegidate() %></li>
	</ul>
	
	<!-- 
	페이지영역에 저장된 속성값은 새로운 페이지에 대한 요청이 발생되면 속성값이 
	모두 파괴되어 소멸된다. 즉 다른 페이지와는 속성이 공유되지 않는다.
	 -->
	<h2>페이지 이동</h2>
	...
	<br />
	<!-- 
		외부파일을 인클루드 하는것은 해당 파일을 원본 그대로 현재문서에 포함시킨후
		컴파일되므로 include지시어를 사용하는경우 같은 페이지로 취급되어 페이지영역이 공유된다.
	 -->
