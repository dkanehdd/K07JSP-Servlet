...
<body>
	...
	*/
	...
	<!-- 최초 실행시에는 파라미터가 없으므로 0으로 간주되어 계산한다.
		만약  해당페이지?myNumber=30    => 결과 40
...
<!-- EL에서는 JSP에서 선언한 변수는 직접 사용할수 없다. 값이 출력되지 않고 null로 인식하게된다. -->
...
<%-- 	\${varScriptLet + 100 } : ${varScriptLet + 100 } <br /> --%>
	<!-- 
	JSP에서 선언한 변수를 EL에서 사용해야 할 경우에는 JSTL의 set태그를 이용해서
	변수를 선언한다. JSP에서 선언후 바로 사용하려면 영역에 저장해야 한다.
	 -->
	...
	<!-- 
		...
		String클래스의 conpareTo()와 같은 방식으로 비교한다.
		즉, 첫번째문자부터 하나씩  비교해나간다. 
	...
	<ul>
		...
		따라서 비교시 객체상태에서 비교가 이루어지게된다.
		100과 90은 실제 숫자의 비교가 이루어진다. -->
		<li>\${fnum > snum } : ${fnum > snum }</li>
		<li>\${100 > 90 } : ${100 > 90 }</li>
		<!-- 
			자바에서는 문자열 비교를할때 equals메소드를 사용하지만
			EL에서는 == 으로 비교가능하다.
		 -->
	...
	<h3>EL의 논리연산자</h3>
	<ul>
		<li>\${5>=5 && 10!=10 } : ${5 ge 5 and 10 != 10}</li>
	...
	<ul>
		...
		<li>${not empty elsizeZero ? "컬렉션에 저장된 객체있음" :
				"컬렉션에 저장된 객체 없음" }</li>
	</ul>
</body>
</html>