...
/* 
checkbox의 경우 여러 항목이 존재할수 있으므로 getParameterValues()를
통해 배열로 받는것이 기본이지만, 항목이 하나만 있는 경우에는 아래와 같이 처리 가능하다.
*/
if("kosmo".equals(userid) && "1234".equals(userpw)){
	...
	/*
	로그인 실패시에는 에러메세지를 리퀘스트영역에 저장한후 메인페이지로
	포워드(페이지 전달)한다. 포워드 된 페이지까지 리퀘스트 영역이 공유되므로
	해당 메세지는 화면에 출력할수 있다.
	*/
